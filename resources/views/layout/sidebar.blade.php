<nav class="sidebar">
    <div class="sidebar-header">
      <a href="#" class="sidebar-brand">
        Fri<span>day</span>
      </a>
      <div class="sidebar-toggler not-active">
        <span></span>
        <span></span>
        <span></span>
      </div>
    </div>
    <div class="sidebar-body">
      <ul class="nav">
        <li class="nav-item nav-category">#</li>
        <li class="nav-item">
          <a href="/" data-link="dashboard" class="nav-link">
            <i class="link-icon" data-feather="compass"></i>
            <span class="link-title">Pano</span>
          </a>
        </li>
        <li class="nav-item nav-category">APP</li>
        <li class="nav-item">
          <a class="nav-link" data-bs-toggle="collapse" href="#projects" role="button" aria-expanded="false" aria-controls="projects">
            <i class="link-icon" data-feather="codepen"></i>
            <span class="link-title">Projeler</span>
            <i class="link-arrow" data-feather="chevron-down"></i>
          </a>
          <div class="collapse" id="projects">
            <ul class="nav sub-menu">
              <li class="nav-item">
                <a href="/project/new" class="nav-link">Yeni Proje</a>
              </li>
              <li class="nav-item">
                <a href="/project/all" class="nav-link">Tüm Projeler</a>
              </li>
            </ul>
          </div>
        </li>

        <li class="nav-item">
          <a class="nav-link" data-bs-toggle="collapse" href="#offers" role="button" aria-expanded="false" aria-controls="offers">
            <i class="link-icon" data-feather="file-text"></i>
            <span class="link-title">Teklifer</span>
            <i class="link-arrow" data-feather="chevron-down"></i>
          </a>
          <div class="collapse" id="offers">
            <ul class="nav sub-menu">
              <li class="nav-item">
                <a href="/offer/new" class="nav-link">Yeni Teklif</a>
              </li>
              <li class="nav-item">
                <a href="/offer/wait" class="nav-link">Bekleyen Teklifler</a>
              </li>
              <li class="nav-item">
                <a href="/offer/all" class="nav-link">Tüm Teklifler</a>
              </li>
            </ul>
          </div>
        </li>

        <li class="nav-item">
          <a class="nav-link" data-bs-toggle="collapse" href="#customers" role="button" aria-expanded="false" aria-controls="customers">
            <i class="link-icon" data-feather="users"></i>
            <span class="link-title">Müşteriler</span>
            <i class="link-arrow" data-feather="chevron-down"></i>
          </a>
          <div class="collapse" id="customers">
            <ul class="nav sub-menu">
              <li class="nav-item">
                <a href="/customer/new" class="nav-link">Yeni Müşteri</a>
              </li>
              <li class="nav-item">
                <a href="/customer/all" class="nav-link">Tüm Müşteriler</a>
              </li>
            </ul>
          </div>
        </li>
      </ul>
    </div>
  </nav>